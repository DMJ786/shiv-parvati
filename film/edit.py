...
REFRAME = {"01": (0.45, 0.55), "09": (0.52, 0.52), "10": (0.38, 0.42), "11": (0.5, 0.5), "15": (0.26, 0.74)}
...
def render(cuts, picks, music, info, vertical, out):
    ...
    total = fr(info["length"])
    card_start = cuts[-1]["end"]
    card_len = total - card_start + DISSOLVE / 2
    last = CLIPS / f"{cuts[-1]['id']}_t{picks[cuts[-1]['id']]}.mp4"
    bg = BUILD / "card_src.png"
    run(["ffmpeg", "-v", "error", "-y", "-sseof", "-0.3", "-i", last, "-frames:v", "1", bg])
    card = render_card(bg, (W, H), card_len, BUILD / f"card_{'v' if vertical else 'h'}.mp4")

    inputs, vf, af = [], [], []
    segs = []
    for k, c in enumerate(cuts):
        clip = CLIPS / f"{c['id']}_t{picks[c['id']]}.mp4"
        pre = DISSOLVE / 2 if k > 0 and cuts[k - 1]["dissolve_out"] else 0
        post = DISSOLVE / 2 if c["dissolve_out"] else 0
        need = c["end"] - c["start"] + pre + post
        clen = duration(clip)
        # Shot 13 must start on its first frame (eyes already open on the bell); others sit a little into the clip.
        a = 0.0 if c["id"] == "13" else max(0.0, min((clen - need) * 0.35, clen - need - 1 / FPS))
        segs.append((clip, fr(a), fr(a + need), pre, post, c))
    segs.append((card, 0.0, card_len, DISSOLVE / 2, 0.0, {"id": "card"}))

    for i, (clip, a, b, pre, post, c) in enumerate(segs):
        inputs += ["-i", clip]
        v = f"[{i}:v]trim=start={a:.4f}:end={b:.4f},setpts=PTS-STARTPTS,fps={FPS},"
        if c["id"] == "card":
            v += "format=yuv420p,setsar=1,settb=AVTB"
        elif vertical:
            x0, x1 = subject_x(clip, a, b, c["id"])
            cw = round(1080 * 9 / 16 / 2) * 2   # 608 px of the 1920x1080 frame
            d = b - a
            xexpr = f"'max(0,min(iw-{cw},({x0:.3f}+({x1 - x0:.3f})*t/{d:.3f})*iw-{cw}/2))'"
        ...
        vf.append(v + f"[v{i}]")
        if c["id"] == "card":
            af.append(f"anullsrc=r=48000:cl=stereo,atrim=0:{b - a:.4f}[a{i}]")
        else:
            af.append(f"[{i}:a]atrim=start={a:.4f}:end={b:.4f},asetpts=PTS-STARTPTS,aresample=48000,"
                      f"aformat=channel_layouts=stereo,afade=t=in:d=0.02,afade=t=out:st={b - a - 0.02:.4f}:d=0.02[a{i}]")

    # Chain: xfade/acrossfade at dissolves, concat at hard cuts. Output time of each join = the planned cut.
    v, a_, t = "[v0]", "[a0]", segs[0][2] - segs[0][1]
    for i in range(1, len(segs)):
        dis = segs[i][3] > 0
        if dis:
            off = t - DISSOLVE
            vf.append(f"{v}[v{i}]xfade=transition=fade:duration={DISSOLVE}:offset={off:.4f},settb=AVTB[vc{i}]")
            af.append(f"{a_}[a{i}]acrossfade=d={DISSOLVE}:c1=tri:c2=tri[ac{i}]")
            t += (segs[i][2] - segs[i][1]) - DISSOLVE
        else:
